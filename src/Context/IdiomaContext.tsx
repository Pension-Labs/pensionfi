...
interface IdiomaContextType {
	idioma: string; // Variable de estado para el idioma actual
	cambiarIdioma: (nuevoIdioma: string) => void; // Función para cambiar el idioma
	textos: Textos; // Esta propiedad depende de tu implementación específica
	handleTraduce: (e: React.ChangeEvent<HTMLInputElement>) => void; // Función para manejar el cambio de idioma
	dropdownAbout: boolean; // Ejemplo de una propiedad booleana
	setDropdownAbout: React.Dispatch<React.SetStateAction<boolean>>; // Función para actualizar dropdownAbout
	dropdownAprende: boolean; // Ejemplo de otra propiedad booleana
	setDropdownAprende: React.Dispatch<React.SetStateAction<boolean>>; // Función para actualizar dropdownAprende
	dropdownProducto: boolean; // Ejemplo de otra propiedad booleana
	setDropdownProducto: React.Dispatch<React.SetStateAction<boolean>>; // Función para actualizar dropdownProducto
	isOpen: boolean; // Ejemplo de una propiedad booleana
	setIsOpen: React.Dispatch<React.SetStateAction<boolean>>; // Función para actualizar isOpen
	handleAllDropdowns: () => void; // Función para manejar todos los dropdowns a la vez
	isChecked: boolean; // Ejemplo de una propiedad booleana
}
type Traducciones = {
	es: {
		menu: {
...
type Textos = Traducciones[keyof Traducciones];
...
const traducciones: Traducciones = {
	es: {
		menu: {
			...
			simulator: 'Simulador reforma previsional',
			pensionfi_simulator: 'Simulador Pensionfi',
			learn: 'Aprende',
			data: 'Datos',
			library: 'Biblioteca',
			contact: 'Contacto',
		...
		home: {
			header: 'Diseñamos, desarrollamos y comunicamos soluciones previsionales',
			newsletter: {
	...
	en: {
		menu: {
			...
			pensionfi_simulator: 'Pensionfi Simulator',
			learn: 'Learn',
			data: 'Data',
			library: 'Library',
			contact: 'Contact',
...
const IdiomaProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
	const [lang, setLang] = useState<string>(getCookie('lang') || 'es'); // Aseguramos que lang sea de tipo string
	const [isChecked, setIsChecked] = useState<boolean>(lang === 'en');
	const [textos, setTextos] = useState<Textos>(
		traducciones[lang as keyof Traducciones]
	); // Aseguramos que textos sea de tipo Textos
	const [dropdownAbout, setDropdownAbout] = useState<boolean>(false);
	const [dropdownAprende, setDropdownAprende] = useState<boolean>(false);
	const [dropdownProducto, setDropdownProducto] = useState<boolean>(false);
	...
	function initTheme() {
		if (!lang) {
			setCookie('lang', 'es');
			setLang('es');
			setTextos(traducciones.es); // No es necesario el type assertion aquí
		} else if (lang === 'en' && 'en' in traducciones) {
			setTextos(traducciones.en); // No es necesario el type assertion aquí
		} else {
			setTextos(traducciones.es); // No es necesario el type assertion aquí
		}
	}
	...
	const handleTraduce = (e: React.ChangeEvent<HTMLInputElement>) => {
		console.log('click');
		const selectedLang = e.target.checked ? 'en' : 'es';
		setIsChecked(selectedLang === 'en');
		setLang(selectedLang);
		setCookie('lang', selectedLang);
		setTextos(traducciones[selectedLang as keyof Traducciones]);
	};
	...
	const data: IdiomaContextType = {
		idioma: lang,
		cambiarIdioma: setLang,
		textos,
		handleTraduce,
		dropdownAbout,
		setDropdownAbout,
